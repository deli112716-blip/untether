  ...
  useEffect(() => {
    if (isActive) {
      if (targetEndTimeRef.current === null) {
        // Set the target end time when focus mode starts
      ...
      }, 500); // Check more frequently, but state only updates if second changes
    } else {
      if (timerRef.current) clearInterval(timerRef.current);
      targetEndTimeRef.current = null; // Reset target on stop
    }
    return () => {
      if (timerRef.current) clearInterval(timerRef.current);
    };
  ...
  const handleFinished = async () => {
    ...
    if ('Notification' in window && Notification.permission === 'granted') {
      new Notification("Focus Complete", {
        body: "Your UnTether focus session has ended. Take a moment to reflect.",
